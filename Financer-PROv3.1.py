...
def process_financial_audit(file_path, client, output_dir, base_currency, rates):
    ...
    system_prompt = (
        ...
        f"Fields: vendor_name, date, amount, currency, currency_symbol, category, description."
    )
    ...
    try:
        ...
        for entry in transactions:
            raw_amount_str = str(entry.get('amount', 0))
            clean_amount = re.sub(r'[^\d.,-]', '', raw_amount_str.replace(',', '.'))

            currency_map = {
                'PLN': 'zł', 'USD': '$', 'EUR': '€', 'GBP': '£',
                'JPY': '¥', 'CHF': 'fr.', 'AUD': 'A$', 'CAD': 'C$',
                'CNY': '元', 'SGD': 'S$', 'INR': '₹', 'NOK': 'kr',
                'AED': 'dh' 
            }
            symbol_to_code = {v: k for k, v in currency_map.items()}
            symbol_to_code.update({
                'ZŁ': 'PLN', 'ZŁOTY': 'PLN', 'PLN': 'PLN', 'POLISH ZLOTY': 'PLN',
                '$': 'USD', 'USD': 'USD', 'DOLLAR': 'USD', 'DOLLARS': 'USD', 'US DOLLAR': 'USD',
                '€': 'EUR', 'EUR': 'EUR', 'EURO': 'EUR', 'EUROS': 'EUR',
                '£': 'GBP', 'GBP': 'GBP', 'POUND': 'GBP', 'POUNDS': 'GBP', 'STERLING': 'GBP',
                'AED': 'AED', 'DIRHAM': 'AED', 'DIRHAMS': 'AED', 'DH': 'AED', 'DHS': 'AED',
                'CHF': 'CHF', 'FRANC': 'CHF', 'FRANCS': 'CHF', 'SWISS FRANC': 'CHF',
                '¥': 'JPY', 'JPY': 'JPY', 'YEN': 'JPY', 'JAPANESE YEN': 'JPY',
                'KČ': 'CZK', 'CZK': 'CZK', 'CORONA': 'CZK',
                'KR': 'NOK', 'NOK': 'NOK', 'SEK': 'SEK', 'DKK': 'DKK', 'KRONA': 'SEK',
                'A$': 'AUD', 'AUD': 'AUD', 'C$': 'CAD', 'CAD': 'CAD',
                '元': 'CNY', 'CNY': 'CNY', 'YUAN': 'CNY',
                '₹': 'INR', 'INR': 'INR', 'RUPEE': 'INR',
                '₿': 'BTC', 'BTC': 'BTC', 'BITCOIN': 'BTC'
            })
            extracted_currency = str(entry.get('currency', '')).upper().strip()
            extracted_symbol = str(entry.get('currency_symbol', '')).strip()

            if extracted_currency in symbol_to_code:
                doc_currency = symbol_to_code[extracted_currency]
            elif extracted_symbol in symbol_to_code:
                doc_currency = symbol_to_code[extracted_symbol]
            else:
                doc_currency = extracted_currency or base_currency

            if doc_currency in currency_map and not extracted_symbol:
                final_symbol = currency_map[doc_currency]
            else:
                final_symbol = extracted_symbol

            try:
                raw_amount = float(clean_amount)
            except ValueError:
                raw_amount = 0.0
                logging.warning(f"Could not parse amount: {raw_amount_str}")
            ...
            try:
                ...
                query = '''
                    INSERT INTO transactions (
                        vendor_name, date, amount, currency, currency_symbol, category, description, 
                ...
                params = (
                    ...
                    processed_entry['currency_symbol'],processed_entry['category'],
                    processed_entry['description'],converted_total, base_currency, rate_to_base,
